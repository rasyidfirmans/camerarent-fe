...
import { ScanLine, X, ReceiptText, MessageCircle } from "lucide-react";
...
type TransactionPopUpProps = {
  ...
  discount: number;
  deliveryFee: number;
  transactionStatus: "Accepted" | "Pending" | "Rejected";
      ...
      <section className="transaction-popup w-[60%] h-[80%] bg-white/10 backdrop-blur-md rounded-xl p-4 border border-slate-300 relative">
        <div className="w-full h-full flex flex-col gap-8 bg-white rounded-xl p-5">
          ...
          <div className="flex flex-row gap-4 overflow-hidden">
            ...
            <div className="button-container flex flex-col gap-4 w-1/4 h-full">
              <Button
                type="button"
                onClick={() => window.open(proofOfPayment, "_blank")}
                variant="flex flex-row items-center justify-center gap-2 bg-primary-blue text-white font-bold py-2 px-4 rounded-lg"
              >
                <ReceiptText />
                Show Payment Proof
              </Button>
              <Button
                type="button"
                onClick={() =>
                  window.open("https://wa.me/+6281343270961", "_blank")
                }
                variant="flex flex-row items-center justify-center gap-2 bg-white text-primary-blue border-4 border-primary-blue font-bold py-2 px-4 rounded-lg"
              >
                <MessageCircle />
                Chat Admin
              </Button>
